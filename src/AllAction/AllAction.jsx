...
function AllAction() {
  ...
  let saveit = localStorage.getItem(`saveitemaction${id}`);
  ...
  let all_action_query = localStorage.getItem(`all_action[${id}]_query`);
  let len = localStorage.getItem(`actions[${id}]length`);
  let len2 = [];
...
const convertStringToBoolean = (value) => {
    ...
    for (let index = 0; index < len2.length; index++) {
      mm.push({
       ...
       firstValue: localStorage.getItem(
          `first-filter-action[${id}][${index}]`
        ),
        SecondValue: localStorage.getItem(
          `second-filter-action[${id}][${index}]`
        ),
        SearchInput: localStorage.getItem(
          `filter-text-action[${id}][${index}]`
        ),
        select: localStorage.getItem(`operation-action[${id}][${index}]`),
        between: convertStringToBoolean(
          localStorage.getItem(`between-action[${id}][${[index]}]`)
        ),
      })
    }
  ...
  useEffect(() => {
    ...
    if (all_action_query) {
      formdata.append("all_action_query", all_action_query.trim());
    }
    else if (seaas) {
      formdata.append("all_action_query", seaas);
    }
    let bodyContent = formdata;
    bodyContent.append("id", id);
    bodyContent.append("event_type", "action");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    addHeading(`Events - Action`);
    // }
  }, [id]);
  useEffect(() => {
    if (resget.data) {
  ...
  const hanldeDelete = (item) => {
    let formData = new FormData();
    formData.append('general', 'rem_pipeline_event');
    formData.append('event_id', item.event_db_id);
    ...
    swal({
      ...
      if (willDelete) {
        apiMethoddelete("postDltEvents", formData)
  ...
  const handleSearch = () => {
    ...
    if (a.includes("") || b.includes("") || c.includes("") || a.includes(null) || b.includes(null) || c.includes(null)) {
      ...
      NotificationArr.map((item, index) => {
        formdata.append(`first-filter-action[${[index]}]`, item.firstValue);
        formdata.append(`second-filter-action[${[index]}]`, item.SecondValue);
        formdata.append(`filter-text-action[${[index]}]`, item.SearchInput);
        formdata.append(
          `operation-action[]`,
          `${item.select.toLowerCase() == "or" ? "OR" : "AND"}
                  `
        );
           localStorage.setItem(`first-filter-action[${id}][${[index]}]`, item.firstValue);
        localStorage.setItem(`second-filter-action[${id}][${[index]}]`, item.SecondValue);
        localStorage.setItem(`between-action[${id}][${[index]}]`, item.between)
        localStorage.setItem(`filter-text-action[${id}][${[index]}]`, item.SearchInput);
        localStorage.setItem(`operation-action[${id}][${index}]`, `${item.select.toLowerCase() == "and" ? "AND" : "OR"}
        `
        );
      });
      localStorage.setItem(`actions[${id}]length`, NotificationArr.length);
      formdata.append(`event-type`, "action");
      let bodyContent = formdata;
  ...
  useEffect(() => {
    if (resSearch.data) {
      localStorage.setItem(`all_action[${id}]_query`, resSearch.data.all_action_query);
      let formdata = new FormData();
      formdata.append("all_action_query", resSearch.data.all_action_query);
  ...
  const handleRemoveSearch = () => {
    let formdata = new FormData();
    localStorage.removeItem(`all_action[${id}]_query`);
    let le = localStorage.getItem(`actions[${id}]length`);
    localStorage.removeItem(`actions[${id}]length`);
    let len3 = [];
    len3.length = parseInt(le)
    for (let index = 0; index < len3.length; index++) {
      localStorage.removeItem(`first-filter-action[${id}][${[index]}]`);
      localStorage.removeItem(`second-filter-action[${id}][${[index]}]`);
      localStorage.removeItem(`filter-text-action[${id}][${[index]}]`);
      localStorage.removeItem(`operation-action[${id}][${[index]}]`);
      localStorage.removeItem(`between-action[${id}][${[index]}]`);
    }
  ...
  const handleSaveSearch = (values) => {
    ...
    let formdata = new FormData();
    formdata.append("all_action_query", vv[1]);
    let bodyContent = formdata;

    bodyContent.append("id", id);
    bodyContent.append("event_type", "action");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    localStorage.setItem(`saveitemaction[${id}]`, values);
    setSaveValue2(true)
    setSaveValue(values)
    setSaveValue3(vv[0])
  };
  ...
  const handleSaveDelete2 = () => {
    let formdata = new FormData();
    formdata.append("all_action_query", "");
    let bodyContent = formdata;
    bodyContent.append("id", id);
    bodyContent.append("event_type", "action");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    localStorage.removeItem(`saveitemaction[${id}]`)
    setSaveValue()
  }
  useEffect(() => {
  ...
  const handleSubmit2 = () => {
    ...
    formdata.append(`all_action_query`, `${localStorage.getItem(`all_action[${id}]_query`).trim()}`);
            ...
            <div className="d-md-flex justify-content-between align-item-center mb-4">
              <ul className="nav nav-tabs b-none">
                ...
                <li className="nav-item">
                  <Link
                    className="nav-link"
                    to={`/${config.ddemoss}createaction/${id}`}
                  >
              ...
              <div>
                <div className="d-flex gap-2">
                  ...
                  {saveValue && <div className="d-flex gap-2">
                    {saveValue2 && <button onClick={() => handleSaveDelete()} className="btn btn-outline-danger" name="delete-old-search-action">Delete Saved search</button>}
            ...
            <div className="d-flex gap-2 my-3">
              ...
              {currentItem === "Kanban" &&
                <>
                  ...
                  {(leadPermission?.super_admin || leadPermission?.action_stages?.active_module !== "0") &&
                    <>
                      {(leadPermission?.super_admin || leadPermission?.action_stages?.create === "1") &&
                        <Link to={`/${config.ddemoss}action_pipelines/${id}/create_stage`} className="btn btn-outline-secondary kanbanbtn"> Create Stage</Link>
                      }
                      {(leadPermission?.super_admin || leadPermission?.action_stages?.view === "1") &&
                        <Link to={`/${config.ddemoss}action_pipelines/${id}/stage_setting`} className="btn btn-outline-secondary kanbanbtn"> Stage Setting</Link>
                      }
                    </>
                  }           </>
              }
