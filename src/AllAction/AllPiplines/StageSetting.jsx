...
const ActionStageSetting = ({ translation }) => {
    ...
    const { data: stageData, loading, error } = useFetch("", `getAllViewEventsStagesSettings/${pipeline_id}&event_type=action`);
    ...
    useEffect(() => {
        if (leadPermission) {
            if (leadPermission?.action_stages?.active_module === "0" || leadPermission?.action_stages?.view === "0") {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${config.apiEndPoint}getAllViewEventsStagesSettings/${pipeline_id}&event_type=action`)
            .then((response) => {
                        ...
                        <div className="card-body">
                            <div className="card-body-title">
                                ...
                                <div className="card-options pipeline-settings">
                                    <Link className="btn btn-sm btn-primary bsm-1 box_shadow" to={`/${config.ddemoss}action_pipelines`} > Go to Pipelines</Link>
                                                                    ...
                                                                    return (
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td>
                                                                                ...
                                                                                {
                                                                                    (leadPermission?.super_admin || leadPermission?.action_stages?.edit === "1")
                                                                                    &&
                                                                                    <Link onClick={() => handleEdit(item)}
                                                                                        className="btn btn-primary btn-sm btn-ed mx-1"
                                                                                        title={Translation(translation, "Edit")}
                                                                                ...
                                                                                {
                                                                                    (leadPermission?.super_admin || leadPermission?.action_stages?.delete === "1")
                                                                                    &&
                                                                                    <Link onClick={() => handleDelete(item)}
                                                                                        className="btn btn-danger btn-sm btn-dl mx-1"
                                                                                        title={Translation(translation, "Delete")}
...
export default ActionStageSetting;
