...
function EditPiplines({ translation }) {
  ...
  const { data: EditPipelines, loading, error } = useFetch({ setDatas }, `getEditEventsPipelines/${id}&event_type=action`);
  ...
  useEffect(() => {
    if (res.data && !res.isLoading) {
      ...
      navigate(`/${config.ddemoss}action_pipelines`);
    }
  }, [res.data]);
    ...
    <div className="container-fluid">
      <div className="row clearfix my-4">
        <div className="col-4 offset-sm-4">
          ...
          <p className="text-center my-4"><Link to={`/${config.ddemoss}action_pipelines`}> Go to Pipelines</Link></p>
        </div>
      </div>
    </div>
  )
