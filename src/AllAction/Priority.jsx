...
export default function Priority({ translation }) {

  const [res, apiMethod] = usePost();
  const [res1, apiMethoddelete] = usePost()
  const { data: getPriorityData, loading, error } = useFetch('', 'getAllViewActionPriority')
  ...
  const handleSubmit = (value, { resetForm }) => {
    ...
    if (value['status_name'] === '') {
      ...
      apiMethod('postCreateActionPriority', formData);
      resetForm();
    }
  }
  useEffect(() => {
  ...
  const handleDelete = (item) => {
    ...
    swal({
      ...
      if (willDelete) {
        apiMethoddelete(`postDeletedActionPriority`, deleteData);
              ...
              <div className="col-8">
                <div className="card">
                  <div className="card-body">
                    <h4 className="card-title">Action Priority List</h4>
                  </div>
                  <table className="table card-table table-vcenter">
                    <tbody>
                      {priorityData && !priorityData.message && priorityData.map((item, index) => {
                        return (
                          <tr key={index}>
                            <td>{Translation(translation, item.priority_label)}</td>{' '}
                            ...
                            <td className="text-right">
                              ...
                              >
                                <Link to={`/${config.ddemoss}action_priority/edit/${item.priority_id}`} className="btn btn-icon vstg" style={{ backgroundColor: '#fff' }}>
