...
const Card = ({ idss, card, stagesEvent, delte, refreshkanban, refreshkanbanToke, pipelineid }) => {
  ...
  const handlestage = () => {
    ...
    formdata2.append("event_type", "action");
            ...
            <Dropdown className="item-action dropdown ml-2">
              ...
              <Dropdown.Menu>
                {(leadPermission?.super_admin || leadPermission?.action_stages?.edit === "1") &&
                  <Link to={`/${config.ddemoss}action_pipelines/${pipelineid && pipelineid}/edit_stage/${card.id}`} className="dropdown-item"  >
