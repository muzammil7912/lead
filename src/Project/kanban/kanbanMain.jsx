...
const Card = ({ idss, card, stagesProject, delte, refreshkanban, refreshkanbanToke }) => {
  ...
  const [{ isover }, dropRef] = useDrop({
    ...
    drop: (item, monitor) => {
      let items = {
        "stage": card.id,
        "project_id": monitor.getItem().project_arrayid,
        "prevstage": monitor.getItem().stageid,

      }
      if (items.prevstage !== items.stage){
      setStageId(card.id);
      setProjectId(monitor.getItem().id);
      setPrevstage(monitor.getItem().stageid)
      setLeadId(monitor.getItem().project_arrayid)
      const StageDiv = document.getElementById('stage_id_' + card.id);
      const mode = 'projects';
      const module = 'Project';
      // console.log(monitor.getItem().project_arrayid, monitor.getItem().id)
      ...
      setSearchval("")
      setLoading(false)
      apiMethod("postProjectsRequiredFields", bodyContent);
      if(card.wonlost == "won") {
        setLoading(true)
        setWon(card.wonlost)
        setNames(monitor.getItem().project_array.fullName)
        axios
        .get(`${config.apiEndPoint}getProjectsWonStageFields/${monitor.getItem().id}&wonStage`,)
        .then((response)=>{
      ...
      if (card.wonlost == "lost") {
        ...
        setWon(card.wonlost)
        axios
          .get(`${config.apiEndPoint}getAllProjectStagesSettings`,)
          .then((response) => {
  ...
  const handlestage = () => {
    ...
    formdata2.append("lead_id",  idLead);
    formdata2.append("pipeline", projectId);
    formdata2.append("general", 'kanban_update_pipeline_stage');
    apiMethod2("postUpdateProjectsStage", formdata2);
  }
  useEffect(() => {
    if (res.data) {
  ...
  useEffect(() => {
    if (stagesProject && card.id) {
      if (stagesProject[card.id] && stagesProject[card.id].length > 0) {
        setStageProjects(stagesProject[card.id]);
  ...
  const handleAssigns = (item, name) => {
    ...
    setShowEdit(true)
    setLoading(true)
    axios.get(`${config.apiEndPoint}getProjectsWonStageFields/${item}&wonStage`,)
      .then((response) => {
        setWondata(response.data);
        setWondata2(response.data.contacts);
        setWondata3(response.data.prospect_stages);
  ...
  function handleSubmit(values) {
    ...
    formdata2.append("lead_id", projectId);
    formdata2.append("mode", "Project");
    ...
    if (vv.length > 0) {
      ...
      handlestage();
      apiMethod3("postProjectsSaveRequiredFields", formdata2);
      if (won == "won" && ass) {
        let formdata3 = new FormData();
        formdata3.append("saveRequiredFields", "true");
        formdata3.append("uProjectType", "assign");
        formdata3.append("userProject", ownerhidden);
        formdata3.append("uProject", projectId);
        formdata3.append("uProjectModule", "Project");
        formdata3.append("pipeline", "Project");
        formdata3.append("assignType", "convert");
        formdata3.append("pProject", assgn);
        formdata3.append("cProject", contact);
        formdata3.append("prop", "project_p");
        apiMethod4("postProjectAssign", formdata3)
      }
    }
    ass = false;
  }
  ...
  const handleAssignL = (item, name) => {
    ...
    setShowEdit2(true)
    axios
      .get(`${config.apiEndPoint}getProjectsWonStageFields/${item}&wonStage`,)
      .then((response) => {
        setSearchval(response.data.assign_user.assigned);
        setOwnerhidden(response.data.assign_user.assigned_to);
        setLoading(false)
  ...
  const handleSubmitAssign = () => {
    let formdata2 = new FormData();
    formdata2.append("uProjectType", "assign");
    formdata2.append("uProject", iDs);
    formdata2.append("userProject", ownerhidden);
    formdata2.append("uProjectModule", "Project");
    apiMethod5("postProjectAssign", formdata2)
    setShowEdit2(false)
  }
  useEffect(() => {
    if (res3.data) {
  ...
  const handleLost = () => {
    if (reason == "") {
      ...
      formdata2.append("mode", "Project");
      formdata2.append("lostreason", `${reason}:${projectId}`);
      apiMethod2("postProjectsLostStage", formdata2);
    }
  }
  return (
    <div
    ...
    >
      ...
      >
        ...
        <div className="card-options align-item-center">
          ...
          {(leadPermission?.super_admin || leadPermission?.project_stages?.active_module !== "0") &&
            <Dropdown className="item-action dropdown ml-2">
              ...
              <Dropdown.Menu>
                {(leadPermission?.super_admin || leadPermission?.project_stages?.edit === "1") &&
                  <Link to={`/${config.ddemoss}project_pipelines/${idss}/edit_stage/${card.id}`} className="dropdown-item"  >
                    <i className="dropdown-icon fa fa-edit"></i>Edit
                  </Link>
                }
                {(leadPermission?.super_admin || leadPermission?.project_stages?.delete === "1") &&
      ...
      <Collapse in={open}>
        ...
        >
          {StageProjects.length ? StageProjects.map((val, index) => {
            return (
              <List
                key={index}
                id={val.project_id}
                ...
                handleAssignProject={(item, name) => handleAssignL(item, name)}
      ...
      <>
        ...
        <Modal show={showEdit2} onHide={handleClose}>
          ...
          <Modal.Body>
            <div className="col-md-12 mt-3">
              <label className="form-label">
                {Translation(translations, "Project Assign to")}
              </label>
              <div ref={ownerRef} className="searchDropDown">
                <input
                  type="text"
                ...
                {resowner.data && (
                  <ul className="list">
                    {resowner.isLoading ? (
                      ...
                      Array.isArray(resowner.data).map((item, index) => {
                        return (
                          <li
                            key={index}
                            onClick={() => handleClick(item)}
            ...
            <div className="modal-footer mt-3">
              ...
              <button type="button" onClick={handleSubmitAssign} className="btn btn-primary btn-projectassignst">Assign Project</button>
            </div>
          </Modal.Body>
        </Modal>
      </>
