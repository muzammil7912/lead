...
const SettingClientStage = ({ translation }) => {
  ...
  const [Prospectdrop, setProspectdrop] = useState();
  const [prosdropVal, setprosdropVal] = useState();
  const [resreadonly, apiMethodreadonly] = usePost();
  const [res, apiMethod] = usePost();
  const [datas, setDatas] = useState("");
  const [datas2, setDatas2] = useState();
  const [options, setoptions] = useState()
  const { data: stages, loading2, error2 } = useFetch('', "getAllClientsStagesPosition");
  const { data: correlationsdb, loading, error } = useFetch({ setDatas }, "getAllClientsStagesSettings");
  const { data: selectedStage, loading3, error3 } = useFetch("", "getAllClientsNewClients");



  const { addHeading } = useContext(MainHeadingContext);
  useEffect(() => {
    addHeading(`Settings - Clients Stages`);
  }, [])
  const { leadPermission } = useContext(MainLeadPermissionContext);
  const navigate = useNavigate();
  useEffect(() => {
    if (leadPermission) {
      if (leadPermission?.client_stages?.active_module === "0" || leadPermission?.client_stages?.view === "0") {
  ...
  const handleDelete = (item) => {
    ...
    swal({
      ...
      if (willDelete) {
        apiMethod(`postDeleteClientsStagesSettings`, bodyContent);
  ...
  function handleSaveSettings() {
    if (StagesFirst != "" && StagesBeforeLast != "" && StagesLast != "" && StagesWon != "" && StagesLost != ""
      ...
      formdata.append("module_stage", "client");
      formdata.append("pipeline_stage", "client");
      ...
      let bodyContent = formdata;
      apiMethod("postUpdateClientsStagesPosition", bodyContent);

      let formdata2 = new FormData();
      formdata2.append("stages", "streadc");
      formdata2.append("readonly", prosdropVal);
      let bodyContent2 = formdata2;
      apiMethodreadonly("postUpdateClientsNewClients", bodyContent2);
    } else {
      swal({
  ...
  function handleSubmit() {
    ...
    if (textinput.current.value == "") {
      ...
      let bodyContent = formdata;
      apiMethod("postCreateClientsStagesSettings", bodyContent);
      handleAPi()
    }
  }
  function handleSubmit2() {
    ...
    let bodyContent = formdata;
    apiMethod("postUpdateClientsStagesSettings", bodyContent);
    handleAPi()
    setReasonDescription("");
    setPublishStatus("");
    setshow1(false)
  ...
  useEffect(() => {
    if (selectedStage) {
      setProspectdrop(selectedStage.prospectStages)
      setprosdropVal(selectedStage.NewClients.id)
    }
  }, [selectedStage]);
  ...
  if (loading || loading3 || loading2) return <Loader />
  return (
    <>
          ...
          <div className="tab-content" id="ex1-content">
            ...
            >
              ...
              >
                ...
                <Tab className="p1" eventKey="New Client - Lead/Prospect Stages" title="New Client - Lead/Prospect Stages">
                  <div className="row">
                    <div className="col-sm-12 col-md-12">
                      <div className="mt-2">
                        <div className="card-body">
                            ...
                            <div className="table-responsive">
                              <table className="table table-hover table-vcenter mb-0 table_custom spacing8 text-nowrap stg">
                                <thead>
                                  ...
                                  <tr>
                                    <td>Lead Stage with New Client</td>
                                    <td className="w200">{selectedStage?.CreateProspect?.body}</td>
                                  </tr>
                                  <tr>
                                    <td>Lead Stage at Create Prospect</td>
                                    <td className="w200">
                                      <select className="form-control" name="position[top]" onChange={(e) => setprosdropVal(e.target.value)} value={prosdropVal}>
                                        <option key={1} value="">-- Select --</option>
                                        {Prospectdrop && Prospectdrop.map((item, i) => (
...
export default SettingClientStage;
