...
const Card = ({ idss,card, stagesEvent, delte, refreshkanban, refreshkanbanToke,Piplineid }) => {
  ...
  const [{ isover }, dropRef] = useDrop({
    ...
    drop: (item, monitor) => {
      ...
      if (card.wonlost == "won") {
        ...
        setWon(card.wonlost)
        axios
          .get(`${config.apiEndPoint}getAllEventStagesSettings`,)
          .then((response) => {
  ...
  const handlestage = () => {
    ...
    formdata2.append("event_type", "meeting");
    ...
    >
      ...
      >
        ...
        <div className="card-options align-item-center">
          ...
          {
            (leadPermission?.super_admin || leadPermission?.meeting_stages?.active_module !== "0") &&
            <Dropdown className="item-action dropdown ml-2">
              ...
              <Dropdown.Menu>
                {(leadPermission?.super_admin || leadPermission?.meeting_stages?.edit === "1") &&
                  <Link to={`/${config.ddemoss}meeting_pipelines/${Piplineid}/edit_stage/${card.id}`} className="dropdown-item">
