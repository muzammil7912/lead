...
const MeetingStageSetting = ({ translation }) => {
    ...
    const { data: stageData, loading, error } = useFetch("", `getAllViewEventsStagesSettings/${pipeline_id}&event_type=meeting`);
    ...
    useEffect(() => {
        if (leadPermission) {
            if (leadPermission?.meeting_stages?.active_module === "0" || leadPermission?.meeting_stages?.view === "0") {
                navigate(`/${config.ddemoss}meeting/${pipeline_id}`);
            }
        }
    }, [leadPermission]);
    ...
    useEffect(() => {
        ...
        axios
            .get(`${config.apiEndPoint}getAllViewEventsStagesSettings/${pipeline_id}&event_type=meeting`)
            .then((response) => {
    ...
    function handleSaveSettings() {
        if (StagesFirst != "" && StagesBeforeLast != "" && StagesLast != "") {
            let formdata = new FormData();
                        ...
                        <div className="card-body">
                            <div className="card-body-title">
                                ...
                                <div className="card-options pipeline-settings">
                                    <Link className="btn btn-sm btn-primary bsm-1 box_shadow" to={`/${config.ddemoss}meeting_pipelines`} > Go to Pipelines</Link>
                                                                ...
                                                                {Array.isArray(listopportunity) ? listopportunity?.map((item, index) => {
                                                                    return (
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td>
                                                                                {
                                                                                    (leadPermission?.super_admin || leadPermission?.meeting_stages?.edit === "1")
                                                                                    &&
                                                                                ...
                                                                                {
                                                                                    (leadPermission?.super_admin || leadPermission?.meeting_stages?.delete === "1")
                                                                                    &&
...
export default MeetingStageSetting;
