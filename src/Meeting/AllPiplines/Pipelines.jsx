...
function Pipelines({ translation }) {
  ...
  const { addMeetingList, Meetinglist } = useContext(MainMeetingListContext);
  const { addHeading } = useContext(MainHeadingContext);
  useEffect(() => {
    addHeading(`Meetings Pipelines`);
  ...
  const handleDelete = (item) => {
    ...
    swal({
      ...
      if (willDelete) {
        ...
        addMeetingList(Meetinglist.filter((ite) => ite.db_id !== item.db_id));
      }
    });
  }
  useEffect(() => {
            ...
            <div className="card">
              ...
              <table className="table card-table table-vcenter" id='table-pipeline'>
                ...
                <tbody className="tbody">
                  {Meetinglist?.length ? Meetinglist.map((item, index) => {
                    return (<tr key={index}>
                      ...
                      <td className="text-right">
                        <div className='d-flex justify-content-end gap-2'>
                          <div className="btn-group">
                            <Dropdown className="btn-group">
                              ...
                              <Dropdown.Menu className="dropdown-menu">
                                <Link className="dropdown-item" to={`/${config.ddemoss}meeting_pipelines/edit/${item.db_id}`}><i className="fa fa-pencil fa-lg"></i> Edit Pipeline</Link>
                          ...
                          <Link to={`/${config.ddemoss}meeting_pipelines/${item.db_id}/create_stage`} className="btn btn-default btn-icon" data-toggle="tooltip" title="" data-original-title="Create New Stage">
                            <i className="fa fa-plus-square fa-lg"></i>
                          </Link>

                          <Link to={`/${config.ddemoss}meeting_pipelines/${item.db_id}/stage_setting`} className="btn btn-default btn-icon" data-toggle="tooltip" title="" data-original-title="Pipelinse Settings"> <i className="fa fa-cog fa-lg"></i> </Link>
                        </div>
                      </td>
                    </tr>)
                  }) : <tr>
