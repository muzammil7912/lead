...
function AllMeeting() {
  ...
  let saveit = localStorage.getItem(`saveitemmeeting[${id}]`);
  ...
  let all_meeting_query = localStorage.getItem(`all_meeting[${id}]_query`);
  let len = localStorage.getItem(`meeting[${id}]length`);
  let len2 = [];
...
const convertStringToBoolean=(value)=>{
  ...
  if (len) {
    ...
    for (let index = 0; index < len2.length; index++) {
      mm.push({
        id: `${index + 1}`,
        firstValue: localStorage.getItem(`first-filter-meeting[${id}][${index}]`),
        SecondValue: localStorage.getItem(`second-filter-meeting[${id}][${index}]`),
        SearchInput: localStorage.getItem(`filter-text-meeting[${id}][${index}]`),
        select: localStorage.getItem(`operation-meeting[${id}][${index}]`),
        between: convertStringToBoolean(localStorage.getItem(`between-meeting[${id}][${[index]}]`)),
  ...
  useEffect(() => {
    ...
    if (all_meeting_query) {
      formdata.append("all_meeting_query", all_meeting_query.trim());
    }
    else if (seaas) {
      formdata.append("all_meeting_query", seaas);
    }
    let bodyContent = formdata;
    bodyContent.append("id", id);
    bodyContent.append("event_type", "meeting");
  ...
  const handleSearch = () => {
    ...
    if (a.includes("") || b.includes("") || c.includes("") || a.includes(null) || b.includes(null) || c.includes(null)) {
      ...
      NotificationArr.map((item, index) => {
        formdata.append(`first-filter-meeting[${[index]}]`, item.firstValue);
        formdata.append(`second-filter-meeting[${[index]}]`, item.SecondValue);
        formdata.append(`filter-text-meeting[${[index]}]`, item.SearchInput);
        formdata.append(
          `operation-meeting[]`,
          `${item.select.toLowerCase() == "or" ? "OR" : "AND"}
                  `
        );
        formdata.append(`event-type`, "meeting");
        localStorage.setItem(`first-filter-meeting[${id}][${[index]}]`, item.firstValue);
        localStorage.setItem(`second-filter-meeting[${id}][${[index]}]`, item.SecondValue);
        localStorage.setItem(`between-meeting[${id}][${[index]}]`, item.between)
        localStorage.setItem(`filter-text-meeting[${id}][${[index]}]`, item.SearchInput);
        localStorage.setItem(`operation-meeting[${id}][${index}]`, `${item.select.toLowerCase() == "and" ? "AND" : "OR"}
        `
        );
      });
      localStorage.setItem(`meeting[${id}]length`, NotificationArr.length);
  ...
  useEffect(() => {
    ...
    if (resSearch.data) {
      localStorage.setItem(`all_meeting[${id}]_query`, resSearch.data.all_meeting_query);
      let formdata = new FormData();
      formdata.append("all_meeting_query", resSearch.data.all_meeting_query);
  ...
  const handleRemoveSearch = () => {
    let formdata = new FormData();
    localStorage.removeItem(`all_meeting[${id}]_query`);
    let le = localStorage.getItem(`meeting[${id}]length`);
    localStorage.removeItem(`meeting[${id}]length`);
    let len3 = [];
    len3.length = parseInt(le)
    for (let index = 0; index < len3.length; index++) {
      localStorage.removeItem(`first-filter-meeting[${id}][${[index]}]`);
      localStorage.removeItem(`second-filter-meeting[${id}][${[index]}]`);
      localStorage.removeItem(`filter-text-meeting[${id}][${[index]}]`);
      localStorage.removeItem(`operation-meeting[${id}][${[index]}]`);
      localStorage.removeItem(`between-meeting[${id}][${[index]}]`);
    }
  ...
  const handleSaveSearch = (values) => {
    ...
    let formdata = new FormData();
    formdata.append("all_meeting_query", vv[1]);
    let bodyContent = formdata;

    bodyContent.append("id", id);
    bodyContent.append("event_type", "meeting");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    localStorage.setItem(`saveitemmeeting[${id}]`, values);
    setSaveValue2(true)
    setSaveValue(values)
    setSaveValue3(vv[0])
  };
  ...
  const handleSaveDelete2 = () => {
    let formdata = new FormData();
    formdata.append("all_meeting_query", "");
    let bodyContent = formdata;
    bodyContent.append("id", id);
    bodyContent.append("event_type", "meeting");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    localStorage.removeItem(`saveitemmeeting[${id}]`)
    setSaveValue()
  }
  useEffect(() => {
  ...
  const handleSubmit2 = () => {
    ...
    formdata.append(`all_meeting_query`, `${localStorage.getItem(`all_meeting[${id}]_query`).trim()}`);
  ...
  return (
    <div className="AllEvents mt-3">
      <div className="container-fluid">
        <div className="card meetingCard">
          <div className="card-headers p-3">
            <div className="d-md-flex justify-content-between align-item-center mb-4">
              <ul className="nav nav-tabs b-none">
                ...
                <li className="nav-item">
                  <Link
                    className="nav-link"
                    to={`/${config.ddemoss}createmeeting/${id}`}
                  >
              ...
              <div>
                <div className="d-flex gap-2">
                  ...
                  {saveValue && <div className="d-flex gap-2">
                    {saveValue2 && <button onClick={() => handleSaveDelete()} className="btn btn-outline-danger" name="delete-old-search-meeting">Delete Saved search</button>}
            ...
            <div className="d-flex gap-2 my-3">
              ...
              {currentItem === "Kanban" &&
               <>
               {(leadPermission?.super_admin || leadPermission?.meeting_stages?.active_module === "1") &&
                 <>
                   {(leadPermission?.super_admin || leadPermission?.meeting_stages?.create === "1") &&
                     <>
                       <Link to={`/${config.ddemoss}meeting_pipelines/${id}/create_stage`} className="btn btn-outline-secondary kanbanbtn">Create Stage</Link>
                     </>
                   }
                   {(leadPermission?.super_admin || leadPermission?.meeting_stages?.view === "1") &&
                     <Link to={`/${config.ddemoss}meeting_pipelines/${id}/stage_setting`} className="btn btn-outline-secondary kanbanbtn">Stage Setting</Link>
                   }
                 </>
               }           </>
              }
              {all_meeting_query && (
                <>
                  <input
                    type="text"
                    className="form-control"
                    name="search_name_field_meeting"
