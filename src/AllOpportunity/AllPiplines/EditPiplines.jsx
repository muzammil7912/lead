...
function EditPiplines({ translation }) {
  ...
  const { id } = useParams();
  const { data: EditPipelines, loading, error } = useFetch({ setDatas }, `getEditOpportunityPiplines/${id}`);
  ...
  useEffect(() => {
    if (res.data && !res.isLoading) {
      ...
      navigate(`/${config.ddemoss}opportunity_pipelines`);
    }
  }, [res.data]);
  ...
  const handleSubmit = (values) => {
    ...
    if (values["pipeline_name"] == "") {
      ...
      apiMethod("postUpdateOpportunityPiplines", formdata);
    }
    ...
    <div className="container-fluid">
      <div className="row clearfix my-4">
        <div className="col-4 offset-sm-4">
          ...
          <p className="text-center my-4"><Link to={`/${config.ddemoss}pipelines`}> Go to Pipelines</Link></p>
        </div>
      </div>
    </div>
  )
