...
if(SaveData) {
  ...
  between: convertStringToBoolean(localStorage.getItem(`between-opportunitiespip[${index}]`)),
  ...
  const handleSearch = () => {
    ...
    if (a.includes("") || b.includes("") || c.includes("") || a.includes(null) || b.includes(null) || c.includes(null)) {
      ...
      NotificationArr.map((item, index) => {
        ...
        localStorage.setItem(`between-opportunitiespip[${index}]`, item.between)
  ...
  const handleRemoveSearch = () => {
    ...
    for (let index = 0; index < len3.length; index++) {
      ...
      localStorage.removeItem(`between-opportunitiespip[${[index]}]`);
}
