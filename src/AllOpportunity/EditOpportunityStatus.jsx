...
function EditOpportunityStatus({ translation }) {
    ...
    const { id } = useParams();
    const { data: EditStatus, loading, error } = useFetch({ setDatas }, `getEditOpportunityStatus/${id}`);
    ...
    useEffect(() => {
        if (res.data && !res.isLoading) {
            ...
            navigate(`/${config.ddemoss}opportunity_status`);
        }
    }, [res.data]);
    ...
    const handleSubmit = (values) => {
        ...
        if (values["pipeline_name"] === "") {
            ...
            formdata.append("type_create", "UpdateStatus");
            apiMethod("postUpdateOpportunityStatus", formdata);
        }
        ...
        <div className="container-fluid">
            <div className="row clearfix my-4">
                <div className="col-4 offset-sm-4">
                    ...
                    <p className="text-center my-4"><Link to={`/${config.ddemoss}opportunity_status`}> Go to Status</Link></p>
                </div>
            </div>
        </div>
    )
...
export default EditOpportunityStatus