...
function CreateLeadStage({ translation }) {
  const { leadPermission } = useContext(MainLeadPermissionContext);
  const navigate = useNavigate();
  useEffect(() => {
    if (leadPermission) {
      if (leadPermission?.lead_stages?.active_module === "0" || leadPermission?.lead_stages?.create === "0") {
  ...
  const handleSubmit = (values, actions) => {
    ...
    if ((values.name != "") && (values.Stage_Name != "") && (values.Backgroun_Color != "") && (values.Font_Color != "") &&
      ...
      apiMethod("postCreateKanbanStages", formdata);
