...
// if(stagess) {
  axios.defaults.headers = {
    ...
    axios.post(`${config.apiEndPoint}postAllViewEventsKanban`,{
      id: id,
      type_event: "follow_up"
    })
    .then((res)=>{
      console.log("asdasdasdsadas",res?.data?.all_stages[0]?.pipeline_id);
      setPipelineID(res?.data?.all_stages[0]?.pipeline_id)
  ...
  <DndProvider backend={HTML5Backend}>
    ...
    <div className='full_lead_stage full_leadnew full_lead_stage_new'>
      {allStages&&(allStages).map(
        (val, i)=>{
          return <Card  pipelineID={PipelineID && PipelineID}  idss={id} key={i} delte={(item) => handeDelete(item)}  card={val} refreshkanbanToke={(item) => handleApi2(item)} stagesEvent={stagesOpportunities} allStages={allStages} refreshkanban={() => handleApi()}  />
        }
        )
      }
    </div>
