...
function AllFollowUp() {
  ...
  let saveit = localStorage.getItem(`saveitemfollowup[${id}]`);
  ...
  let all_follow_up_query = localStorage.getItem(`all_followup[${id}]_query`);
  let len = localStorage.getItem(`followup[${id}]length`);
  let len2 = [];
  ...
  const { data: SearchData, loading, error } = useFetch("", "getEventsSearchColumns");
  const { data: SaveData, loading3, error3 } = useFetch("", "getEventsSaveSearch");
...
const convertStringToBoolean=(value)=>{
  ...
  if (len) {
    ...
    for (let index = 0; index < len2.length; index++) {
      mm.push({
        id: `${index + 1}`,
        firstValue: localStorage.getItem(`first-filter-followup[${id}][${index}]`),
        SecondValue: localStorage.getItem(`second-filter-followup[${id}][${index}]`),
        SearchInput: localStorage.getItem(`filter-text-followup[${id}][${index}]`),
        select: localStorage.getItem(`operation-followup[${id}][${index}]`),
        between: convertStringToBoolean(localStorage.getItem(`between-followup[${id}][${[index]}]`)),
   })
    }
  ...
  useEffect(() => {
    ...
    if (all_follow_up_query) {
      formdata.append("all_follow_up_query", all_follow_up_query.trim());
    }
    else if (seaas) {
      formdata.append("all_follow_up_query", seaas);
    }
    let bodyContent = formdata;
    bodyContent.append("id", id);
    bodyContent.append("event_type", "follow_up");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    // } else {
    // }
    // return () => {
  ...
  const hanldeDelete = (item) => {
    ...
    valdelate.append("event_id", item.event_db_id);
    valdelate.append("general", "rem_pipeline_event");
    ...
    swal({
      ...
      if (willDelete) {
        apiMethoddelete("postDltEvents", valdelate)
        setDatas(datas.filter((ite) => ite.event_db_id !== item.event_db_id));
      }
    });
  ...
  const practiceProfileRender = {
    ["Grid"]: <GridView respremetio={leadPermission} data={datas} deletee={(item) => hanldeDelete(item)} loadingss={resget.isLoading} />,
  ...
  const handleSearch = () => {
    ...
    if (a.includes("") || b.includes("") || c.includes("") || a.includes(null) || b.includes(null) || c.includes(null)) {
      ...
      NotificationArr.map((item, index) => {
        formdata.append(`first-filter-follow_up[${[index]}]`, item.firstValue);
        formdata.append(`second-filter-follow_up[${[index]}]`, item.SecondValue);
        formdata.append(`filter-text-follow_up[${[index]}]`, item.SearchInput);
        formdata.append(
          `operation-follow_up[]`,
          `${item.select.toLowerCase() == "or" ? "OR" : "AND"}
                  `
        );
        localStorage.setItem(`first-filter-followup[${id}][${[index]}]`, item.firstValue);
        localStorage.setItem(`second-filter-followup[${id}][${[index]}]`, item.SecondValue);
        localStorage.setItem(`between-followup[${id}][${[index]}]`, item.between)
        localStorage.setItem(`filter-text-followup[${id}][${[index]}]`, item.SearchInput);
        localStorage.setItem(`operation-followup[${id}][${index}]`, `${item.select.toLowerCase() == "and" ? "AND" : "OR"}
        `
        );
      });
      localStorage.setItem(`followup[${id}]length`, NotificationArr.length);
      formdata.append(`event-type`, "follow_up");
      let bodyContent = formdata;
      apiMethodSearch("postEventsSearch", bodyContent);
    }
  };
  useEffect(() => {

    if (resSearch.data) {
      localStorage.setItem(`all_followup[${id}]_query`, resSearch.data.all_follow_up_query);
      let formdata = new FormData();
      formdata.append("all_follow_up_query", resSearch.data.all_follow_up_query);
      formdata.append("id", id);
      formdata.append("event_type", "follow_up");
      apiMethodGet("postViewPiplineEvents", formdata);
    }
  }, [resSearch])

  const handleRemoveSearch = () => {
    let formdata = new FormData();
    localStorage.removeItem(`all_followup[${id}]_query`);
    let le = localStorage.getItem(`followup[${id}]length`);
    localStorage.removeItem(`followup[${id}]length`);
    let len3 = [];
    len3.length = parseInt(le)
    for (let index = 0; index < len3.length; index++) {
      localStorage.removeItem(`first-filter-followup[${id}][${[index]}]`);
      localStorage.removeItem(`second-filter-followup[${id}][${[index]}]`);
      localStorage.removeItem(`filter-text-followup[${id}][${[index]}]`);
      localStorage.removeItem(`operation-followup[${id}][${[index]}]`);
      localStorage.removeItem(`between-followup[${id}][${[index]}]`);
    }
  ...
  const handleSaveSearch = (values) => {
    ...
    let formdata = new FormData();
    formdata.append("all_follow_up_query", vv[1]);
    let bodyContent = formdata;

    bodyContent.append("id", id);
    bodyContent.append("event_type", "follow_up");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    localStorage.setItem(`saveitemfollowup[${id}]`, values);
    setSaveValue2(true)
    setSaveValue(values)
    setSaveValue3(vv[0])
  };
  const handleSaveDelete = () => {
    ...
    apiMethodSaveDelete("deleteEventsSaveSearch", formdata)
  }
  const handleSaveDelete2 = () => {
    let formdata = new FormData();
    formdata.append("all_follow_up_query", "");
    let bodyContent = formdata;
    bodyContent.append("id", id);
    bodyContent.append("event_type", "follow_up");
    apiMethodGet("postViewPiplineEvents", bodyContent);
    localStorage.removeItem(`saveitemfollowup[${id}]`)
    setSaveValue()
  }
  useEffect(() => {
  ...
  function submit1(page,pageSize){
    ...
    apiMethoddata('postViewPiplineEvents', formdata);
  ...
  const handleSubmit2 = () => {
    ...
    formdata.append(`all_followup_query`, `${localStorage.getItem(`all_followup[${id}]_query`).trim()}`);
    apiMethodGetS("postEventsFilterSaved", formdata);

  };
  useEffect(() => {
    if (resgetS.data && !resgetS.isLoading) {
      axios.get(`${config.apiEndPoint}getEventsSaveSearch/${id}`)
        .then((response) => {
  ...
  if (loading || loading2 || loading3 || loading4 || respremetion.isLoading) return <Loader />;
  ...
  return (
    <div className="AllEvents mt-3">
      <div className="container-fluid">
        <div className="card followupCard">
          <div className="card-headers p-3">
            <div className="d-md-flex justify-content-between align-item-center mb-4">
              <ul className="nav nav-tabs b-none">
                ...
                <li className="nav-item">
                  <Link
                    className="nav-link"
                    to={`/${config.ddemoss}createfollowup/${id}`}
                  >
              ...
              <div>
                <div className="d-flex gap-2">
                  ...
                  {saveValue && <div className="d-flex gap-2">
                    {saveValue2 && <button onClick={() => handleSaveDelete()} className="btn btn-outline-danger" name="delete-old-search-followup">Delete Saved search</button>}
            ...
            <div className="d-flex gap-2 my-3">
              ...
              {currentItem === "Kanban" &&
                <>
                  {(leadPermission?.super_admin || leadPermission?.follow_up_stages?.active_module === "1") &&
                    <>
                      {(leadPermission?.super_admin || leadPermission?.follow_up_stages?.create === "1") &&
                        <Link to={`/${config.ddemoss}followup_pipelines/${id}/create_stage`} className="btn btn-outline-secondary kanbanbtn"> Create Stage</Link>
                      }
                      {(leadPermission?.super_admin || leadPermission?.follow_up_stages?.view === "1") &&
                        <Link to={`/${config.ddemoss}followup_pipelines/${id}/stage_setting`} className="btn btn-outline-secondary kanbanbtn"> Stage Setting</Link>
                      }
                    </>
                  }           </>
              }
              {all_follow_up_query && (
                <>
                  <input
                    type="text"
                    className="form-control"
                    name="search_name_field_followup"
