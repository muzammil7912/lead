...
function CreateStage({ translation }) {
  ...
  const [OrdernumberVal, setOrdernumberVal] = useState('');
  ...
  useEffect(() => {
    if (res?.data?.message) {
      ...
      navigate(`/${config.ddemoss}followup/${id}/Kanban`)
    }
  }, [res.data]);
  useEffect(() => {
    if (leadPermission) {
      if (leadPermission?.follow_up_stages?.active_module === "0" || leadPermission?.follow_up_stages?.create === "0") {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${config.apiEndPoint}getEditEventsPipelines/${id}&event_type=follow_up`)
      .then((response) => {
